...
//supplemental info on plot:
	...
	legXSec->Draw();
        ...
        lat->SetTextFont(42);
        lat->SetTextSize(ltxSetTextSize2);
        lat->DrawLatex(ratio_ltxText2_xStart,ratio_ltxText2_yStart,"#splitline{|y|<2.4}{10<p_{T}<50 GeV/c}");
  ...
  // gPad->RedrawAxis();
   ...
   if(bSavePlots)
   {
       if (whichPlot==0)
       {
           pc1->SaveAs(Form("%s/pdf/xsec_vsCent_%d.pdf",outputDir,CentMin));
           pc1->SaveAs(Form("%s/png/xsec_vsCent_%d.png",outputDir,CentMin));
       }else{
	 pc1->SaveAs(Form("%s/pdf/ratio_vsCent_ref%d_%d.pdf",outputDir,drawRef,CentMin));
	 pc1->SaveAs(Form("%s/png/ratio_vsCent_ref%d_%d.png",outputDir,drawRef,CentMin));
        }
   }
